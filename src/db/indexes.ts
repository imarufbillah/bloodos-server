...
const safeCreateIndex = async (
  ...
  try {
    ...
    if (error.code === 85 || error.code === 86) {
      console.log(
        `  ℹ Index ${options.name} already exists or has conflict - skipping`,
      );
      return;
    }
    throw error;
...
export const createIndexes = async (db: Db): Promise<void> => {
  ...
  try {
    ...
    // ========================================================================
    const bloodRequestsCollection = db.collection(
      COLLECTION_NAMES.BLOOD_REQUESTS,
    );
    ...
    // ========================================================================
    const contactAuditLogsCollection = db.collection(
    ...
    // ========================================================================
    const adminActionLogsCollection = db.collection(
      COLLECTION_NAMES.ADMIN_ACTION_LOGS,
    );
  ...
  try {
    ...
    for (const { name, expectedCount } of collections) {
      ...
      if (customIndexCount >= expectedCount) {
        ...
        console.warn(
          `  ⚠ ${name}: Expected ${expectedCount} indexes, found ${customIndexCount}`,
        );
        allValid = false;
      }
    }
